export default async function handler(req, res) {
  const { id } = req.query;
  if (!id) return res.status(400).json({ error: 'Missing draft id' });

  const PAT = process.env.AIRTABLE_PAT;
  const BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN;
  const BASE = 'app3fWPgMxZlPYSgA';
  const TABLE = 'tbl8JFtBNsbfO89KQ'; // Pending Drafts

  try {
    const response = await fetch(
      `https://api.airtable.com/v0/${BASE}/${TABLE}/${id}`,
      { headers: { 'Authorization': `Bearer ${PAT}` } }
    );
    const data = await response.json();

    if (data.error) return res.status(404).json({ error: 'Draft not found' });

    const f = data.fields || {};

    // Resolve photo URLs — permanent Drive URLs first, Telegram as fallback
    let photos = [];

    // 1. Use image_url field (permanent Drive URLs, set by Update Draft Image URL node)
    if (f.image_url) {
      photos = f.image_url.split(',').map(s => s.trim()).filter(Boolean);
    }

    // 2. If no permanent URLs, try Telegram file resolution (works ~1 hour)
    if (photos.length === 0 && f.photo_file_id && BOT_TOKEN) {
      const fileIds = f.photo_file_id.split(',').map(s => s.trim()).filter(Boolean);
      for (const fid of fileIds) {
        try {
          const fr = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/getFile?file_id=${fid}`);
          const fd = await fr.json();
          if (fd.ok && fd.result.file_path) {
            photos.push(`https://api.telegram.org/file/bot${BOT_TOKEN}/${fd.result.file_path}`);
          }
        } catch(e) {}
      }
    }
    // No fallback stock photo — if there's no image, show none

    // Build IG caption with hashtags
    let igCaption = f.content_ig || '';
    if (f.hashtags_ig && !igCaption.includes('#')) {
      igCaption += '\n\n' + f.hashtags_ig;
    }

    const preview = {
      posts: [{
        label: f.topic || f.content_brief?.substring(0, 60) || 'Post Preview',
        photos: photos,
        instagram: igCaption ? { caption: igCaption } : null,
        gbp: f.content_gbp ? { body: f.content_gbp } : null,
        facebook: (f.content_fb || igCaption) ? { caption: f.content_fb || igCaption } : null
      }]
    };

    res.setHeader('Access-Control-Allow-Origin', '*');
    res.status(200).json(preview);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
}
